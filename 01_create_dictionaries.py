...
import configs.data.config_tts_all_05 as config_tts_all_05
    ...
    for i in range(len(retVal)):
        ...
        for ii in range(len(temp_compressions)):
            if temp_compressions[ii] is not None:
                compr = np.random.choice(temp_compressions[ii]["compression"], 1)[0]
                if temp_compressions[ii]["format"] != "encodec":
                    compr = int(compr)
                compressions.append({"format": temp_compressions[ii]["format"], "compression": compr})
            else:
                compressions.append(None)

        retVal[i]["noise"] = noise
        retVal[i]["compressions"]= compressions
...
def main():
    ...
    for d_config in configs:
        ...
        if d_config.silence_path_train != '':
